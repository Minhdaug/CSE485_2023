...
--
-- Máy chủ: 127.0.0.1
-- Thời gian đã tạo: Th9 18, 2024 lúc 12:24 PM
...
-- Thủ tục
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_DSBaiViet` (IN `ten_the_loai` VARCHAR(255))   BEGIN
    DECLARE cnt INT;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_music`  AS SELECT `b`.`ma_bviet` AS `ma_bviet`, `b`.`tieude` AS `tieude`, `b`.`ten_bhat` AS `ten_bhat`, `b`.`tomtat` AS `tomtat`, `b`.`ngayviet` AS `ngayviet`, `t`.`ten_tloai` AS `ten_tloai`, `a`.`ten_tgia` AS `ten_tgia` FROM ((`baiviet` `b` join `theloai` `t` on(`b`.`ma_tloai` = `t`.`ma_tloai`)) join `tacgia` `a` on(`b`.`ma_tgia` = `a`.`ma_tgia`))  ;
